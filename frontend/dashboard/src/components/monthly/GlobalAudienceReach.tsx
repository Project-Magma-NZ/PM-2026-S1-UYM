...
import { fetchCountryReach } from '../../services/analytics';
import type { CountryReach } from '../../types';

const WORLD_TOPO_URL = 'https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json';

interface Props { yearMonth?: string; }

const GlobalAudienceReach = ({ yearMonth }: Props) => {
  const [countries, setCountries] = useState<CountryReach[]>(COUNTRY_REACH);

  useEffect(() => {
    fetchCountryReach(yearMonth).then(setCountries).catch(() => {});
  }, [yearMonth]);
        ...
        <div className="relative bg-slate-900 rounded-3xl overflow-hidden min-h-[240px] flex items-center justify-center group">
          ...
          <div className="absolute bottom-5 left-5 right-5 bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-2xl flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] text-white/60 font-bold uppercase tracking-widest">Top Country</p>
                <p className="text-sm font-bold text-white">
                  {countries[0]?.name ?? '—'} {countries[0] ? `${countries[0].percentage}%` : ''}
                </p>
              </div>
            </div>
